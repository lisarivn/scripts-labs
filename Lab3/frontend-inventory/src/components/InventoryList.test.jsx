...
describe('Компонент InventoryList', () => {
    ...
    test('фільтрує товари за назвою', async () => {
        render(<InventoryList />);
        ...
        const searchInput = screen.getByPlaceholderText('Пошук товару...');

        fireEvent.change(searchInput, {
            target: { value: 'серверна' },
        });

        expect(screen.getByText(/Серверна стійка/i))
            .toBeInTheDocument();

        expect(screen.queryByText(/Маршрутизатор/i))
